...
get_title() {
    ...
    response=$(curl -s "$api_url") || {
        notify-send "NTSmenu" "Failed to fetch NTS API"
    ...
    case "$1" in
        ...
        "NTS Radio 2") 
            ...
            ;;
        *) 
            notify-send "NTSmenu" "Error: Invalid station '$1'"
...
get_current_stream(){
    if pgrep -f "mpv .*--title=NTS Radio 1" >/dev/null; then
        ...
        notify-send "NTSmenu" "No NTS stream currently playing"
...
get_details() {
    ...
    local response
    response=$(curl -s "$api_url") || {
        notify-send "NTSmenu" "Failed to fetch NTS API details"
        return 1
    }
...
show_details() {
    ...
    # Parse JSON output from get_details()
...
get_tracklist() {
    ...
    local response
    response=$(curl -s "$api_url") || {
        notify-send "NTSmenu" "Failed to connect to NTS API"
    ...
    tracklist_json_url=$(echo "$response" | jq -r ".results[$((station-1))].now.embeds.details.links[] | select(.rel==\"tracklist\") | .href") || {
        notify-send "NTSmenu" "Could not find tracklist URL"
    ...
    tracklist_data=$(curl -s "$tracklist_json_url") || {
        notify-send "NTSmenu" "Failed to fetch tracklist"
...
SoulSeek (unavailable)"
    ...
    # Present menu (using here-document for cleaner input)
    local website
    website=$(rofi -dmenu -p "Select site to search" -i <<< "$search_sites") || {
        notify-send "NTSmenu" "Search cancelled"
        return 1
    }
    ...
    case $website in 
        ...
        "SoulSeek (unavailable)"*) notify-send "NTSmenu" "SoulSeek search under construction" ;;
        *) 
            notify-send "NTSmenu" "No site selected. Canceling search."
            echo "NTSmenu" "No site selected. Cancelled search." ;;
    esac
}
...
show_tracklist() {
    ...
    if [[ -z "$tracklist" || "$tracklist" == "null" ]]; then
        # Display empty state in rofi's message area
...
id_song() {
    ...
    current_default=$(pactl info | awk 'NR == 13 {print $3}').monitor || {
        notify-send "NTSmenu" "Failed to get audio source"
        echo "Failed to get audio source" >&2
        return 1
    }

    # Identify song
    notify-send "NTSmenu" "Listening..."
    local current_song
    current_song=$(setsid -f songrec recognize -d "$current_default" 2>/dev/null) || {
        notify-send "NTSmenu" "Song recognition failed"
    ...
    if [[ -n "$current_song" ]]; then
        ...
        notify-send "NTSmenu" "Identified:\n$current_song"
        echo "Successfully identified: $current_song"
    else
        notify-send "NTSmenu" "No song detected"
...
pass_link_handler() {
    #option not working for some reason --onset-metadata=title="'$stream_tit'" \
    local stream_tit
    stream_tit=$(get_title "$selection") || return 1
    
    if ! setsid -f mpv \
        --title="$selection - NTSmenu - mpa" \
        --force-media-title="$stream_tit - $selection" \
        "${streams["$selection"]}"
    then
        notify-send "NTSmenu" "Failed to open '$selection'"
        echo "Failed to open '$selection'" >&2
        return 1
    fi
    
    notify-send "NTSmenu" "Opening..." "'$selection'"
    echo "Opening '$selection'"
}

# kills current radio via simple pkill command
kill_radio() {
    if ! pkill -f "mpv --title=.* - NTSmenu.*"; then
        notify-send "NTSmenu" "Failed to kill current stream"
        echo "failed to kill '$selection'" >&2
    #else
        #notify-send "NTSmenu" "no active stream found"
    fi
    #notify-send "NTSmenu" "killed current stream"
